  ...
  const runOcr = async (imageBase64: string) => {
    ...
    try {
      const res = await fetch("/api/ocr", {
        ...
        body: JSON.stringify({ imageBase64, mode: "diesel" }),
      });
      ...
      try {
        ...
        setOcrErrorDetail(
          raw
            ? `Përgjigje jo-JSON (${res.status}): ${raw.slice(0, 400)}`
            : `Përgjigje e zbrazët (status ${res.status})`
        );
    ...
    <div className="px-4 pt-6">
      ...
      {showForm && (
        ...
        >
          ...
          />
          <div className="relative bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full max-w-lg lg:max-w-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 pb-8 pt-4">
              ...
              <div className="mb-6">
                ...
                {ocrErrorDetail && (
                  <div
                    className="mt-4 rounded-xl border-2 border-red-200 bg-red-50 px-4 py-3 text-sm"
                    role="alert"
                  >
                    <p className="font-bold text-red-900 mb-1">
                      {t.diesel.aiErrorTechnical}
                    </p>
                    <pre className="font-mono text-xs text-red-800 whitespace-pre-wrap break-all max-h-40 overflow-y-auto">
                      {ocrErrorDetail}
                    </pre>
                  </div>
                )}
